...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  """
  - take dot product of every outside word and and center word and sum them all
  - run softmax on result
  - loss = -log * result[outside_word_idx]

  - expectation = softmaxed_result_i * u_i *outside vector i* for each i
  - dJ / dv_c = u_o *outside vector* - expectation
  
  - dj / dU = v_c * (y_hat - y)T
    = center_word vector * (softmaxed_result - [1 if i == outside_word_index else 0 for i in len(outside_vectors)]
  
  """
  # Calculate Loss
  dot_product_results = np.array([np.dot(outside_vector, center_word_vec) for outside_vector in outside_vectors])
  softmaxed_results = softmax(dot_product_results)
  loss = -1 * np.log(softmaxed_results[outside_word_idx])

  # Calculate dJ / dv_c meaning gradient with respect to center word
  temp = np.array([np.dot(softmaxed_results[i], outside_vectors[i]) for i in range(len(outside_vectors))])
  expectation = sum(temp)
  grad_center_vec = (-1 * outside_vectors[outside_word_idx]) + expectation

  # Calculate dJ/dU meaning gradient with respect to outside vectors
  y = np.array([1 if i == outside_word_idx else 0 for i in range(len(outside_vectors))])
  tmp = softmaxed_results - y
  grad_outside_vecs = center_word_vec * tmp.reshape(len(tmp), 1)
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for word in outside_words:
    ...
    loss_current, grad_center_vec_current, grad_outside_vecs_current = word2vec_loss_and_gradient(center_word_vec=center_word_vec, outside_word_idx=outside_word_index, outside_vectors=outside_vectors, dataset=dataset)
    loss += loss_current
    grad_center_vecs[center_word_idx] += grad_center_vec_current
    grad_outside_vectors += grad_outside_vecs_current
  ### END CODE HERE
...
def sgd(f, x0, step, iterations, postprocessing=None, use_saved=False,PRINT_EVERY=10):
  ...
  for iter in range(start_iter + 1, iterations + 1):
    ...
    ### START CODE HERE
    loss, grad = f(x)
    amount_to_change = step * grad
    x = x - amount_to_change

    if iter % PRINT_EVERY == 0:
      print(f"loss: {loss}, grad: {grad}, amount_to_change: {amount_to_change} x: {x}")
    ### END CODE HERE
