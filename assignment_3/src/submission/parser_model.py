...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # Input size is num features (each word vector) times the embedding size
        # Input word embedding are just concatenated into the input x vector
        in_features = self.n_features * self.embed_size
        first_linear = torch.nn.Linear(in_features=in_features, out_features=hidden_size)
        torch.nn.init.xavier_uniform_(first_linear.weight, gain=1)
        self.embed_to_hidden = first_linear

        self.dropout = torch.nn.Dropout(p=self.dropout_prob)

        hidden_linear = torch.nn.Linear(in_features=hidden_size, out_features=self.n_classes)
        torch.nn.init.xavier_uniform_(hidden_linear.weight, gain=1)

        self.hidden_to_logits = hidden_linear
    ...
    def embedding_lookup(self, t):
        ...
        x = t_embeddings.view(batch_size, self.n_features * self.embed_size)
        ### END CODE HERE
        return x
    ...
    def forward(self, t):
        ...
        torch.nn.functional.relu(h, inplace=True)
