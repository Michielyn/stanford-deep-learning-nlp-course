class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            # marks the second (second most recently added) item on the stack as a dependent of the first item
            # and removes the second item from the stack.
            last_stack_index = len(self.stack) - 1
            second_to_last_stack_index = len(self.stack) - 2

            last_token = self.stack[last_stack_index]
            second_to_last_token = self.stack.pop(second_to_last_stack_index)

            self.dependencies.append((last_token, second_to_last_token))

        elif transition == "RA":
            #  marks the first (most recently added) item on the stack as a dependent of the second item and
            # removes the first item from the stack.
            last_stack_index = len(self.stack) - 1
            second_to_last_stack_index = len(self.stack) - 2

            second_to_last_token = self.stack[second_to_last_stack_index]
            last_token = self.stack.pop(last_stack_index)

            self.dependencies.append((second_to_last_token, last_token))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    """
    Try waiting until you have applied all of the parse steps in the mini batch before you remove anything from unfinished parses. I suspect that the way you are removing things is hitting an edge case
    """

    while unfinished_parses:
        minibatch_parses = unfinished_parses[:batch_size]
        predictions = model.predict(minibatch_parses)
        indexes_to_remove = []
        for i in range(len(minibatch_parses)):
            parse = minibatch_parses[i]
