...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lower_limit = 4
        upper_limit = int(self.block_size * 7 / 8)
        trunc_length = random.randint(lower_limit, upper_limit)
        trunc_doc = document[:trunc_length]
        # print(trunc_doc)

        import math
        if random.randint(0, 1):
            masked_content_length = random.randint(0, math.floor(trunc_length/2))
        else:
            masked_content_length = random.randint(0, math.ceil(trunc_length/2))
        prefix_length = random.randint(0, trunc_length-masked_content_length)

        prefix = trunc_doc[:prefix_length]
        masked_content = trunc_doc[prefix_length:masked_content_length + prefix_length]
        suffix = trunc_doc[masked_content_length + prefix_length:]

        masked_string_unpadded = f"{prefix}{self.MASK_CHAR}{suffix}{self.MASK_CHAR}{masked_content}"
        num_pads_needed = self.block_size - len(masked_string_unpadded)
        masked_string = f"{masked_string_unpadded}{self.PAD_CHAR * num_pads_needed}"
        ...
        and output strings as Long tensors and return the resulting data point.
        """
        input_tensor = torch.Tensor([self.stoi[char] for char in list(input_string)]).type(torch.LongTensor)
        output_tensor = torch.Tensor([self.stoi[char] for char in list(output_string)]).type(torch.LongTensor)
