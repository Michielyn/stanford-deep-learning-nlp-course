...
if args.function == 'pretrain':
    ...
    wiki_dataset = dataset.CharCorruptionDataset(
        open(args.pretrain_corpus_path, encoding='utf-8').read(),
        block_size=block_size,
    )
    trainer = trainer.Trainer(model, wiki_dataset, None, pretrain_tconf)
    trainer.train()
    ...
    ### START CODE HERE
    name_dataset = dataset.NameDataset(open(args.finetune_corpus_path, encoding='utf-8').read(),
                                       pretrain_dataset)

    using_pretrained = bool(args.reading_params_path)
    if args.reading_params_path:
        model.load_state_dict(torch.load(args.reading_params_path))

    ###     2. Finetune the model on this corpus

    # IF WITHOUT PRETRAINED MODEL
    ...
    # USING PRETRAINED
        ...
        tconf = trainer.TrainerConfig(
            max_epochs=10, batch_size=256, learning_rate=6e-4,
            lr_decay=True, warmup_tokens=512 * 20, final_tokens=200 * len(pretrain_dataset) * block_size,
            num_workers=4
        )
        trainer = trainer.Trainer(model, name_dataset, None, tconf)
    ...
    # model.load_state_dict(torch.load("vanilla.pretrain.params", map_location=torch.device('cpu')))
    # torch.save(model.state_dict(), args.writing_params_path)
    ### END CODE HERE
    pass
